...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // REQUIEM
    // VSEM PRIVET YA REQUEIM PIDORAS
    ...
    const ImRect bb(ImVec2(window->DC.CursorPos.x + 136, window->DC.CursorPos.y), ImVec2(window->DC.CursorPos.x + 336 ,window->DC.CursorPos.y + (w, label_size.y + style.FramePadding.y * 2.0f)));
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(total_bb.Min.x - 136, bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::BeginComboDef(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const float preview_width = ((flags & ImGuiComboFlags_WidthFitPreview) && (preview_value != NULL)) ? CalcTextSize(preview_value, NULL, true).x : 0.0f;
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : ((flags & ImGuiComboFlags_WidthFitPreview) ? (arrow_size + preview_width + style.FramePadding.x * 2.0f) : CalcItemWidth());
    const ImRect bb(ImVec2(window->DC.CursorPos.x + 136, window->DC.CursorPos.y), ImVec2(window->DC.CursorPos.x + 282, window->DC.CursorPos.y + (w, label_size.y + style.FramePadding.y * 2.0f)));
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &bb))
        return false;
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        // SetNextWindowContentSize(ImVec2(272, 248));
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoLabel))
    {
     ...
     // if (!(1 == 1 & ImGuiColorEditFlags_NoSidePreview))
     // {
